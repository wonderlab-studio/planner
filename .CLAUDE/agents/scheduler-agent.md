...
## Охват сбора карточек для планирования

Алгоритм должен собирать карточки из двух источников, не только из вчерашней колонки:
- Вчерашняя колонка — стандартная логика `_run_regular`
- Сегодняшняя колонка — карточки с `event_time.date() < today` (устаревшая дата события)

Карточки второго источника нужно очищать от event_time и добавлять в общий пул перед phase1. Иначе они становятся "слепыми пятнами": в phase1 не попадают (дата не совпадает), в remaining тоже не попадают (алгоритм их не видит) — остаются нетронутыми с устаревшей датой навсегда.

Признак устаревшей карточки: находится в сегодняшней колонке, имеет `event_time`, но `event_time.date() < today`.

## Порядок фаз алгоритма и маршрутизация по event_time

При добавлении или изменении фаз строго соблюдай порядок:

**phase0** (резервирование, `event_time == today`) → **phase1** (фиксированные события, `event_time == today` → секция) → **phase1б** (будущие события, `event_time > today` → целевая колонка, исключить из пула) → **phase2-3** (распределение по приоритетам).

Карточки с `event_time.date() > today` в phase1б переносятся в колонку нужного дня и исключаются из сегодняшнего пула — иначе они получат слот сегодня, хотя событие запланировано на другой день. Когда добавляешь новую фазу между существующими, сначала нарисуй полную последовательность фаз и убедись, что каждая карточка попадает ровно в одну ветку.
