...
- КРИТИЧНО: раздел "На контроле" в `MORNING_SYSTEM` должен содержать слово "ОБЯЗАТЕЛЬНО" (напр. "ОБЯЗАТЕЛЬНО включи раздел На контроле") — иначе Claude иногда пропускает этот раздел в ответе.
